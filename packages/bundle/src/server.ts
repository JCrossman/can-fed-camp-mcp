...
export function serverInstructions(): string {
  ...
  return [
    ...
    "",
    "Photos: get_site_details returns a site's photos as native image content.",
    "Describe in words what they show (how",
    "exposed, treed, level, or private the site looks) so the citizen gets the",
    "picture even if their host does not render tool-result images. Say the photos",
    "were returned; never claim they are already visible in the citizen's UI.",
    "",
